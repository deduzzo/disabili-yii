        ...
        foreach ($sheets as $sheet) {
            ...
            foreach ($values as $index => $row) {
                if ($index > 1) {
                    ...
                    if (isset($row[3]) && $row[3] !== "" && str_contains(strtoupper(trim($sheetTitle)), strtoupper(trim($row[3]))) && str_contains(strtolower(trim($row[0])), "positiv") && isset($row[24]) && trim($row[24]) !== "") {
                        $count++;

...
class IseeController extends Controller
{
    ...
    public function actionAggiungiDaIstanza()
    {
        ...
        if ($istanza && $this->request->isPost && ($data['tipologia'] === 'maggiore' || $data['tipologia'] === 'minore')) {
            $iseeCorrente = Isee::find()->where(['id_istanza' => $istanza->id, 'valido' => true])->one();
            ...
            $model->valido = $iseeCorrente === null || ( !$iseeCorrente->anno_riferimento || ($iseeCorrente->anno_riferimento >= $model->anno_riferimento));
